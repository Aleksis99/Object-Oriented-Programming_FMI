...
// ...
```
Разбира се, ето по-кратки версии на условията.

---

### Задача 1: Клас `Person`

Да се създаде клас `Person` с полета за име и години.

*   **Имплементирайте "Голямата петорка"** (конструктори, деструктор, оператор=) за правилно управление на динамичната памет за името (дълбоко копиране и преместване).
*   **Добавете `set`/`get` методи** за полетата.

---

### Задача 2: Клас `Club`

Да се създаде клас `Club` с име и масив от членове (`Person*`).

*   **Имплементирайте "Голямата петорка"** за управление на динамичната памет за името и масива с членове.
*   **Реализирайте методи:**
    *   `addMember`: Добавя член, ако има място.
    *   `deleteMember`: Премахва член по индекс.
    *   `increasemaxMembers`: Увеличава капацитета на клуба.
    *   `printClub`: Отпечатва информация за всички членове.