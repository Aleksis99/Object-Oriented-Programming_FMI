...
Изброените типове (enumerations) в C++ се използват за дефиниране на набор от наименувани константи, което прави кода по-четим и лесен за поддръжка. В C++ съществуват два основни вида: традиционните `enum` и въведените в C++11 `enum class`.

---

## 1. Традиционен `enum` (Unscoped Enumeration)

Това е класическият C-стил `enum`. Въпреки че е полезен, той има някои съществени недостатъци, свързани с обхвата (scope) и безопасността на типовете.
...
Color myColor = Red; // Директен достъп до константата
```

**Основни характеристики и проблеми:**
* **Изтичане на обхвата (Scope Leakage):** Имената на константите (`Red`, `Green`) "изтичат" в обхвата, в който е дефиниран самият `enum`. Това означава, че не можете да имате друг `enum` в същия обхват със същите имена (напр. `enum TrafficLight { Red, Yellow, Green };` ще даде грешка при компилация заради `Red`).
* **Неявно преобразуване (Implicit Conversion):** Стойностите на традиционния `enum` автоматично се преобразуват до цели числа (`int`). Това може да доведе до логически грешки, ако случайно сравните `Color` с числа или с друг `enum`.

---

## 2. Строг `enum class` (Scoped Enumeration)

Въведени в стандарта C++11, `enum class` (или `enum struct`) решават проблемите на традиционните изброени типове. Те осигуряват силна типизация и строг обхват.
...
enum class Color {
    Red, // Няма проблем с името Red, защото е в отделен scope
    Blue
};

TrafficLight light = TrafficLight::Red; // Задължително се указва името на класа
```

**Основни предимства:**
* **Строг обхват (Strong Scoping):** Константите са видими само чрез оператора за принадлежност `::`. `TrafficLight::Red` и `Color::Red` са напълно различни и не си пречат.
* **Няма неявно преобразуване (No Implicit Casting):** `enum class` не се конвертира автоматично до `int`. Ако се опитате да напишете `int val = TrafficLight::Red;`, компилаторът ще даде грешка. За да вземете числовата стойност, трябва да използвате `static_cast<int>(TrafficLight::Red)`.
...
**Задача 2:**  Напишете функция, която кпира съдържанието на един файл в друг.
...
2.  Да се напише функция, която от име на файл да запише съдържанието му в друг файл.
